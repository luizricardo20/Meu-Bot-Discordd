...
async def criar_loja(ctx):
    ...
    # ---------- COMANDO 3: ULTRA COMUNIDADE (Social) ----------
@bot.command()
@commands.has_permissions(administrator=True)
...
# ---------- COMANDO 5: ULTRA STREAMER (Criadores de Conteúdo) ----------
@bot.command()
@commands.has_permissions(administrator=True)
async def criar_streamer(ctx):
    ...
    for nc in cargos: await ctx.guild.create_role(name=nc)
    cat_avisos = await ctx.guild.create_category("📺 ━━ MURAL DA LIVE ━━")
    await ctx.guild.create_text_channel("🚨┃lives-on", category=cat_avisos)
    await ctx.guild.create_text_channel("🎬┃videos-novos", category=cat_avisos)
    await ctx.guild.create_text_channel("📅┃agenda-e-horarios", category=cat_avisos)
    ...
    await ctx.guild.create_text_channel("💎┃sub-exclusive", category=cat_chat)
    await ctx.guild.create_text_channel("💡┃ideias-de-videos", category=cat_chat)
    ...
    await ctx.guild.create_voice_channel("🔊 Jogando com Subs", category=cat_calls)
    await ctx.guild.create_voice_channel("🔒 Sala de Gravação", category=cat_calls)
    # ---------- COMANDO 6: ULTRA DESENHO (Artes/Ilustração) ----------
@bot.command()
@commands.has_permissions(administrator=True)
async def criar_desenho(ctx):
    ...
    await ctx.guild.create_voice_channel("🔊 Desenhe Comigo", category=cat_calls)

# ---------- COMANDO 7: ULTRA MARKETING (Dropshipping/PLR) ----------
@bot.command()
@commands.has_permissions(administrator=True)
async def criar_marketing(ctx):
    ...
    for nc in cargos: await ctx.guild.create_role(name=nc)
    cat_mural = await ctx.guild.create_category("🎯 ━━ ESTRATÉGIAS ━━")
    ...
    await ctx.guild.create_voice_channel("🔊 Mastermind Geral", category=cat_calls)

# ---------- COMANDO 8: ULTRA DESIGN (Edição Visual) ----------
@bot.command()
@commands.has_permissions(administrator=True)
async def criar_design(ctx):
    ...
    await ctx.guild.create_voice_channel("🔊 Call Co-Working", category=cat_calls)
    # ---------- COMANDO 9: ULTRA TECNOLOGIA (Hardware/Setups) ----------
@bot.command()
@commands.has_permissions(administrator=True)
async def criar_tecnologia(ctx):
    ...
    await ctx.guild.create_voice_channel("🔊 Bancada de Testes", category=cat_calls)

# ---------- COMANDO 10: ULTRA CINEMA (Filmes/Séries) ----------
@bot.command()
@commands.has_permissions(administrator=True)
...
# ---------- COMANDO 11: ULTRA ANIME (Cultura Otaku) ----------
@bot.command()
@commands.has_permissions(administrator=True)
async def criar_anime(ctx):
    await limpar_servidor(ctx)
    cargos = ["👑 Hokage Mestre", "⛩️ Otaku Lendário", "🏮 Cosplayer VIP", "🌸 Membro Otaku", "👤 Civil"]
    for nc in cargos: await ctx.guild.create_role(name=nc)
    ...
    cat_conversas = await ctx.guild.create_category("💬 ━━ ALDEIA GERAL ━━")
    ...
    await ctx.guild.create_text_channel("🔮┃recomendações", category=cat_conversas)
    cat_calls = await ctx.guild.create_category("🎙️ ━━ LOUNGES JAPÃO ━━")
    await ctx.guild.create_voice_channel("🔊 Distrito Kawaii", category=cat_calls)
    await ctx.guild.create_voice_channel("🎵 Abidings Animes (Música)", category=cat_calls)

# ---------- COMANDO 12: ULTRA RPG (Guildas/Mesas) ----------
@bot.command()
@commands.has_permissions(administrator=True)
async def criar_rpg(ctx):
    await limpar_servidor(ctx)
    cargos = ["👑 Mestre Lendário", "⚔️ Guerreiro Veterano", "🔮 Mago Supremo", "🏹 Arqueiro", "👤 Aventureiro"]
    for nc in cargos: await ctx.guild.create_role(name=nc)
    cat_mural = await ctx.guild.create_category("📜 ━━ QUADRO DE MISSÕES ━━")
    ...
    cat_aventura = await ctx.guild.create_category("🛡️ ━━ TAVERNA DO REINO ━━")
    ...
    await ctx.guild.create_text_channel("🔒┃sala-dos-mestres", category=cat_aventura)
    cat_calls = await ctx.guild.create_category("🎙️ ━━ CAMPANHAS COLETIVAS ━━")
    await ctx.guild.create_voice_channel("🎲 Mesa de RPG 01", category=cat_calls)
